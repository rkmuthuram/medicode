...
export default {
  ...
  methods: {
    async getWidgetSeven() {
  try {
   const response = await this.axios.get('https://backend.medicodesolution.com/staging/dashboardAnalytics/w7')
 this.totalPatient = response.data.totalPatients;
 this.totalSession = response.data.totalSessions;
 this.data = response.data.caseTable;
  } catch (error) {
    console.error(error);
  }
},
