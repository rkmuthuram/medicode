<template>
  <div>
    <ol class="breadcrumb">
      ...
      <li class="breadcrumb-item active">Update Vehicle - {{data.title}}</li>
    </ol>
...
export default {
  ...
  data() {
    return {
     vehicleId: this.$route.params.vehicleId,
      ...
      isLoading:false,
       dropzoneOptions: {
          url: 'https://backend.medicodesolution.com/staging/vehicles/upload',
          thumbnailWidth: 150,
          maxFilesize: 5.0,
           maxFiles: 5,
          addRemoveLinks: true,
  ...
  methods: {
    navigateToList(){
        this.$router.push({name:'VehiclesListing'});
    },
    removeCoverImage(i){
 ...
 self.axios.patch('https://backend.medicodesolution.com/staging/vehicles/'+ this.vehicleId, {
...
async getService() {
  try {
   const response = await this.axios.get('https://backend.medicodesolution.com/staging/vehicles/'+ this.vehicleId)
   this.data = response.data.vehicleInfo[0];
   this.images = response.data.vehicleInfo[0].images.split(',');
   this.cover_image[0] = response.data.vehicleInfo[0].cover_image_url;
  } catch (error) {
    console.error(error);
  }
}
