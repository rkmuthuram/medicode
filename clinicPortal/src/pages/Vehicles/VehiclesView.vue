<template>
  <div>
    <ol class="breadcrumb">
      ...
      <li class="breadcrumb-item active">Vehicle Dashboard - {{data.title}}</li>
    </ol>
...
export default {
  ...
  methods: {
    navigateToUpdate(){
        this.$router.push({name:'VehiclesUpdate', params: { vehicleId :this.vehicleId}});
    },
    activateService(){
         var self = this;
       self.isLoading = true;
      self.$validator.validateAll().then((result) => {
		  if (result) {
                self.axios.patch('https://backend.medicodesolution.com/staging/vehicles/status/'+this.vehicleId, {
                   status:1
                })
