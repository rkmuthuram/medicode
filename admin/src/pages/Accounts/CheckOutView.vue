   ...
   <legend>Complete Information</legend>
    ...
    <div class="mb-lg">
       ...
       <div class="mb-lg">
          ...
          On {{data.update_time}}  | Reason: {{data.rollback_comments}}
          </div>
          ...
          <br><br>
            ...
            <b-button type="submit" variant="danger" class="btn-rounded" v-if="isLoading==false && data.rollback!='true'" @click="rollbackStock">
              Rollback Completely
                ...
                <b-button type="submit" variant="warning" class="btn-rounded" v-if="isLoading==false" @click="ammendStock">
              Ammend
              </b-button>
        </b-tabs>
      </b-col>
     ...
     <b-tabs class="mb-lg" v-if="showAmmend==true">
          <b-tab title="Ammend Checkout Record" active>
   <form-wizard
          shape="tab"
          color="#3498db"
          title=""
...
export default {
  ...
  methods: {
   async ammendStock(){
     var self=this;
     if(self.data.rollback!='true'){
          return    Messenger().post({ type:'error',message :'Perform rollback first before making ammendment!', hideAfter: 3,showCloseButton:true});
     }
     else {
       self.showAmmend = true;
        return    Messenger().post({ type:'success',message :'Continue with ammendment..', hideAfter: 3,showCloseButton:true});
     }
   },
...
//  console.log(this.allSelectedProducts);
 ...
 self.axios.post('https://backend.enigmedsvcs.com/development/checkout/submit', {
          ...
          staffId:"JEGA",
           staffName:"Enigma Administrator",
           ammend:true,
           create_time:self.data.create_time,
           ammendId:self.data.id
  ...
  try {
   ...
   async getCheckOut() {
  try {
   const response = await this.axios.get('https://backend.enigmedsvcs.com/development/checkout/'+ this.checkOutId)
   this.data = response.data.checkOutInfo[0];
  this.allProducts = JSON.parse(this.data.allProducts);
  this.allSelectedProducts = this.allProducts;
 ...
 async rollbackStock() {
  var self=this;
  if(self.rollback_comments=='' || self.rollback_comments.length<5){
    return   Messenger().post({ type:'error',message :'Valid Rollback reason required!', hideAfter: 3,showCloseButton:true});
  }
      var finalProducts = new Array();
...
//  console.log(this.allSelectedProducts);
 ...
 if(index==length){
try {
   const response = await this.axios.post('https://backend.enigmedsvcs.com/development/rollback/checkout/'+ this.checkOutId, {
          finalProducts:finalProducts,
          rollback_comments:self.rollback_comments
    })
                .then(function (response) {
