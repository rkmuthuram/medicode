<template>
  <div>
    <ol class="breadcrumb">
      ...
      <li class="breadcrumb-item active">Shop Product Dashboard - {{data.title}}</li>
    </ol>
...
<h3> Other Images </h3>
   ...
   <legend>Product Status </legend>
     <b-badge variant="success" v-if="data.status==1">STATUS : ACTIVE PRODUCT</b-badge><b-badge variant="danger" v-if="data.status==0">STATUS : PRODUCT SUSPENDED</b-badge> <br><br>
...
export default {
  ...
  methods: {
    navigateToUpdate(){
        this.$router.push({name:'ShopUpdate', params: { shopId :this.shopId}});
    },
    activateService(){
         var self = this;
       self.isLoading = true;
      self.$validator.validateAll().then((result) => {
		  if (result) {
                self.axios.patch('https://backend.enigmedsvcs.com/development/shop/status/'+this.shopId, {
                   status:1
                })
